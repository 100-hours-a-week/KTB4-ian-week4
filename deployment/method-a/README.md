...
Ubuntu 서버 24.04 LTS x86_64 한 대에 Java 21, Nginx, MySQL,
...
- `scripts/03-configure-mysql.sh`: 루프백 MySQL과 앱 계정
- `scripts/mysql-migration-access.sh`: 배포 중에만 Flyway DDL 권한
- `scripts/configure-backend-env.sh`: 비밀값 비노출 환경 파일 생성
- `scripts/install-operations.sh`: 재부팅 후에도 사용할 운영 도구 영구 설치
- `scripts/04-deploy-backend.sh`: 버전 JAR과 심볼릭 링크
- `scripts/05-deploy-frontend.sh`: 정적 파일 버전과 심볼릭 링크
- `scripts/06-configure-systemd.sh`: 강화된 서비스
- `scripts/07-configure-nginx.sh`: SPA 및 역방향 프록시
- `scripts/08-configure-free-domain-https.sh`: Dynu·TLS·보안 쿠키 전환
- `scripts/update-dynu.sh`: EC2 공개 IPv4 변경을 Dynu에 반영
- `scripts/deploy.sh`: 배포 적용
- `scripts/rollback.sh`: 이전 백엔드/프론트엔드 동시 롤백
- `scripts/backup.sh`, `restore.sh`: DB·업로드 백업/복구
- `scripts/verify.sh`: 서비스·포트·H2·권한 검사
- `docs/AWS_CONSOLE_GUIDE.md`: 콘솔 수동 절차
...
않는다. 실제 비밀값은 예제·Git·로그·증빙에 기록하지 않는다.
