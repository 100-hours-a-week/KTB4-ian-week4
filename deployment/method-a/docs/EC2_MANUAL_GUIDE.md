...
모든 명령은 사용자가 AWS 콘솔의 세션 관리자에서 직접 실행한다.
명령 출력에 비밀값을 포함하지 말고 `env`, `printenv`, 실제 환경 파일,
SQL 덤프를 공유하지 않는다.
...
S3는 사용하지 않는다. 세션 관리자에는 일반적인 브라우저 파일
업로드 기능이 없으므로, 별도의 승인된 전송 경로가 없다면 임시 SCP를
사용한다.

사용자가 직접 수행:

1. EC2 상세 정보에서 키 페어 이름을 확인
2. 해당 비공개 키를 실제로 보유한 경우에만 다음 단계 진행
3. 키 페어가 없거나 비공개 키를 보유하지 않았다면 22를 열지 말고 중단
4. 보안 그룹의 22를 현재 공인 IP `/32`에만 임시 허용
5. 전용 비공개 키 권한을 로컬에서 `600`으로 제한
6. 아래 자리표시자 명령을 사용자 로컬 터미널에서 실행
...
7. 전송 직후 보안 그룹의 22 규칙 삭제
8. 이후 작업은 세션 관리자만 사용

비공개 키나 실제 공개 IP를 Codex에 전달하지 않는다.
...
- 아키텍처: `x86_64`
- 루트 디스크: 약 20GiB
- 초기 8080·3306 외부 수신 주소 없음

실패 확인:

- `aarch64`이면 잘못된 AMI이므로 중단
- 디스크가 20GiB보다 작거나 암호화 여부가 콘솔에서 불명확하면 중단
...
- Java 21 JDK 헤드리스, Nginx, MySQL 설치
...
별도의 JWT 비밀값, 프론트엔드 출처를 숨김 입력한다. 출처에는 스킴,
호스트와 선택적 포트만 입력하고 마지막 `/`, 경로, 쿼리, 프래그먼트를 붙이지
않는다. 실제 값은 출력하지 않는다.
...
HTTP IP로 임시 검증하면 보안 쿠키가 비활성화된다는 경고가 나온다.
도메인과 TLS 적용 후 다시 실행해 HTTPS 출처와 보안 쿠키를
활성화한다.
...
- MySQL, `community-backend`, Nginx 활성
...
로그를 전달할 때 인증 정보, 쿠키, 토큰, 사용자 데이터, DB URL의
사용자명을 마스킹한다.

## 8. 기능 검증

공개 기능 검증 전 무료 고정 도메인과 HTTPS를 적용한다. 먼저 Dynu에서
호스트를 `pulse`로 입력하고 사용할 최상위 도메인을 선택해 무료 호스트 이름을 만든다.
생성된 전체 주소(`pulse.<선택한-suffix>`)를 확인하고, 계정 비밀번호와 다른
별도 IP 업데이트 비밀번호를 `My Account > Change Username/Password`에서
설정한다. 이후 보안 그룹의 80·443을
`0.0.0.0/0`에 허용한 뒤:
...
Dynu 전체 호스트 이름, Let's Encrypt 알림 이메일, Dynu IP 업데이트 비밀번호를
차례로 입력한다. 비밀번호는 숨김 입력 후 즉시 SHA-256 처리되며 원문은
저장하지 않는다. 해시는 `/etc/community/dynu.env`에 `root:root 600`으로만
저장된다.
...
백업/복구 관리 작업은 Ubuntu MySQL의 로컬 루트 소켓 인증을 사용하며
애플리케이션은 MySQL 루트 계정을 사용하지 않는다. 백업은 개인정보를
포함할 수 있으며 Git·증빙에 첨부하지 않는다.

## 11. 복구

복구 전 별도 최신 백업 존재 여부를 확인한다. 사용자가 정확한 파일을
선택한 뒤 실행:
...
`RESTORE_ARCHIVE`는 비밀값이 아니지만 승인된 백업 경로여야 한다.
...
재부팅으로 공개 IP가 바뀌지 않는 경우가 많지만 중지/시작 시에는
바뀔 수 있다. Dynu 타이머가 같은 `pulse` 호스트 이름에 새 IP를 반영할 때까지
